...
use geometry::transform::{Transform, TransformMut};
...
pub trait IsBundle: Clone + FlatLen + Flatten<PortGeometry> + Transform + Send + Sync {}

impl<T: Clone + FlatLen + Flatten<PortGeometry> + Transform + Send + Sync> IsBundle for T {}
...
impl TransformMut for PortGeometry {
    fn transform_mut(&mut self, trans: Transformation) {
        self.primary.transform_mut(trans);
        self.unnamed_shapes
            .iter_mut()
            .for_each(|shape| shape.transform_mut(trans));
        self.named_shapes
            .iter_mut()
            .for_each(|(_, shape)| shape.transform_mut(trans));
    }
}

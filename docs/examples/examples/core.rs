...
impl Schematic for BufferInlineHardMacro {
    ...
    type NestedData = ();
    fn schematic(&self, ctx: &Context) -> substrate::error::Result<Self::NestedData> {
...
mod try_data {
    ...
    impl Schematic for Vdivider {
        type Io = super::VdividerIo;
        type Schema = Spice;
        type NestedData = ();
        fn schematic(
            &self,
            ctx: &Context,
        ) -> substrate::error::Result<substrate::schematic::Cell<Self>> {
            let cell = substrate::context::prepare_cell_builder(None, ctx.clone(), self);

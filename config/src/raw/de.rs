...
pub(super) struct Deserializer<'config> {
    ...
    /// SUBSTRATE_PROFILE_DEV_BUILD_OVERRIDE_ prefixes exist. But
    /// SUBSTRATE_BUILD_TARGET should not check for prefixes because it would
    /// collide with SUBSTRATE_BUILD_TARGET_DIR. See `ConfigMapAccess` for
    /// details.
...
impl<'de, 'config> de::MapAccess<'de> for ConfigMapAccess<'config> {
        ...
        V: de::DeserializeSeed<'de>,
    {
        ...
        // Example:
        //     SUBSTRATE_BUILD_TARGET
        //     SUBSTRATE_BUILD_TARGET_DIR
        // or
        //     SUBSTRATE_PROFILE_DEV_DEBUG
        //     SUBSTRATE_PROFILE_DEV_DEBUG_ASSERTIONS
        ...
        // starting with SUBSTRATE_FOO_BAR_. This is a problem for keys like
        // SUBSTRATE_BUILD_TARGET because checking for a prefix would incorrectly
        // match SUBSTRATE_BUILD_TARGET_DIR. `deserialize_option` would have no
        // choice but to call `visit_some()` which would then fail if
        // SUBSTRATE_BUILD_TARGET isn't set. So we check for these prefixes and
        // disallow them here.
    ...
    fn new(de: Deserializer<'config>) -> Result<ValueDeserializer<'config>, ConfigError> {
        ...
        let definition = {
            ...
            match (de.config.env.contains_key(env), de.config.get_cv(&de.key)?) {
                ...
                // SUBSTRATE_FOO_BAR_* where `SUBSTRATE_FOO_BAR` is not set.
                (_, None) => env_def,
            }
        };

...
impl<L> AtollLayer for PdkLayer<L> {
    fn dir(&self) -> RoutingDir {
        self.inner.dir()
    }
...
impl<L: AtollLayer> LayerSlice<'_, L> {
...
impl<R: AsRef<S::Layer> + AtollLayer, S: substrate::layout::schema::Schema> Draw<S>
    for DebugRoutingGrid<R>
{
    fn draw(self, recv: &mut DrawReceiver<S>) -> substrate::error::Result<()> {
        for layer in self.grid.start..self.grid.end {
            for track in 0..self.max_coord(layer) {
                ...
                let shape = Shape::new(self.grid.stack.layer(layer).as_ref().clone(), r);
                recv.draw(shape)?;
            }
        }
        Ok(())

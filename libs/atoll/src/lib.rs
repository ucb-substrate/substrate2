...
use fold::FoldedArray;
use serde::{Deserialize, Serialize};
use std::any::Any;
use std::collections::HashMap;
use std::ops::Deref;
use substrate::types::layout::LayoutBundle;
...
use substrate::types::schematic::{
    IoNodeBundle, IoTerminalBundle, Node, NodeBundle, SchematicBundleKind,
};
...
pub struct Instance<T: Schematic + Layout> {
...
impl<T: Schematic + Layout> Instance<T> {
    /// Translates this instance by the given XY-coordinates in LCM units.
    pub fn translate_mut(&mut self, p: Point) {
    ...
    /// Aligns this instance with another instance.
    pub fn align_mut<T2: Schematic + Layout>(
        &mut self,
        other: &Instance<T2>,
        mode: AlignMode,
        offset: i64,
    ...
    /// Aligns this instance with another instance with the same top layer.
    pub fn align<T2: Schematic + Layout>(
        mut self,
        other: &Instance<T2>,
        mode: AlignMode,
        offset: i64,
...
pub struct TileBuilder<'a, S: schematic::schema::Schema + layout::schema::Schema> {
    nodes: IndexMap<Node, NodeInfo>,
    connections: ena::unify::InPlaceUnificationTable<NodeKey>,
    schematic: &'a mut schematic::CellBuilder<S>,
    /// The layout builder.
    pub layout: &'a mut layout::CellBuilder<S>,
    /// The layer stack.
    pub layer_stack: Arc<LayerStack<PdkLayer<S::Layer>>>,
    ...
    via_maker: Option<Arc<dyn ViaMaker<S::Layer>>>,
...
struct TileBuilderUnused<'a, S: schematic::schema::Schema + layout::schema::Schema> {
    #[allow(dead_code)]
    schematic: &'a mut schematic::CellBuilder<S>,
    /// The layout builder.
    pub layout: &'a mut layout::CellBuilder<S>,
...
pub struct DrawnInstance<T: Schematic + Layout> {
...
impl TileAbstractBuilder {
    fn finalize_abstract(self) -> (Abstract, Vec<Path>) {
        ...
        let mut abs = InstanceAbstract::merge(abs, top_layer, outline, port_ids, assigned_nets);
...
impl<'a, S: schematic::schema::Schema + layout::schema::Schema> TileBuilder<'a, S> {
    /// Splits off data not required for building an abstract.
    fn split_for_abstract(
        self,
        port_nodes: Vec<Node>,
        outline: Rect,
    ) -> (TileAbstractBuilder, TileBuilderUnused<'a, S>) {
    ...
    fn new<T: Flatten<Node>>(
        schematic_io: &'a T,
        schematic: &'a mut schematic::CellBuilder<S>,
        layout: &'a mut layout::CellBuilder<S>,
    ) -> Self {
        let layer_stack = layout
            .ctx()
            .get_installation::<LayerStack<PdkLayer<S::Layer>>>()
            .unwrap();
    ...
    /// and [`Layout`].
    pub fn generate_primitive<B: Clone + AtollPrimitive<Schema = S>>(
        &mut self,
        block: B,
    ) -> Instance<B> {
        ...
        let abs = Abstract::generate(
            &self.layout.ctx,
            layout.raw_cell(),
            B::outline(&layout.cell()),
        );
    ...
    pub fn generate_primitive_connected<
        B: Clone + AtollPrimitive<Schema = S>,
    ...
    pub fn generate<B: Clone + Tile<Schema = S>>(&mut self, block: B) -> Instance<TileWrapper<B>> {
        ...
        let abs_path =
            atoll_ctx
                ...
                .generate(block.clone(), move |block| {
                    ...
                    let mut layout_cell = layout::CellBuilder::new(ctx_clone);
                    let mut cell = TileBuilder::<'_, S>::new(
                        &schematic_io,
                        &mut schematic_cell,
                        &mut layout_cell,
                    );
    ...
    pub fn generate_connected<B: Clone + Tile<Schema = S>, C>(
        &mut self,
        block: B,
        io: C,
    ...
    /// and [`Layout`].
    pub fn generate_primitive_named<B: Clone + AtollPrimitive<Schema = S>>(
        &mut self,
        block: B,
    ...
    /// Generates a named ATOLL instance from a Substrate block that implements [`Schematic`]
    /// and [`Layout`] and connects its IO to the given bundle.
    pub fn generate_primitive_connected_named<B: Clone + AtollPrimitive<Schema = S>, C>(
        &mut self,
        block: B,
        io: C,
        name: impl Into<ArcStr>,
    ...
    pub fn generate_named<B: Clone + Tile<Schema = S>>(
        &mut self,
        block: B,
        name: impl Into<ArcStr>,
    ) -> Instance<TileWrapper<B>> {
    ...
    /// Generates a named ATOLL instance from a block that implements [`Tile`]
    /// and connects its IO to the given bundle.
    pub fn generate_connected_named<B: Clone + Tile<Schema = S>, C>(
        &mut self,
        block: B,
        io: C,
        name: impl Into<ArcStr>,
    ...
    pub fn draw<B: Schematic<Schema = S> + Layout<Schema = S>>(
        &mut self,
        instance: Instance<B>,
    ) -> substrate::error::Result<DrawnInstance<B>> {
        let physical_loc = instance.physical_loc();

        let io_nodes: Vec<Node> = instance.io().flatten_vec();
        let parent_net_ids = io_nodes
            .iter()
        ...
        let orig_bbox = instance
            ...
            .grid
            .slice()
            .lcm_to_physical_rect(instance.raw.abs.lcm_bounds);
    ...
    pub fn signal<K: HasBundleKind<BundleKind: SchematicBundleKind>>(
        ...
        let bundle = self.schematic.signal(name, kind);
    ...
    /// Sets the via maker.
    pub fn set_via_maker<T: Any + ViaMaker<S::Layer>>(&mut self, via_maker: T) {
...
pub trait Tile: Block<Io: HasBundleKind<BundleKind: SchematicBundleKind>> {
    ...
    ///
    /// Returns the schematic/layout data, the layout bundle, and the ATOLL outline rectangle.
    fn tile<'a>(
        &self,
...
impl<T: Tile> Schematic for TileWrapper<T> {
    type Schema = <T as Tile>::Schema;
    type NestedData = <T as Tile>::NestedData;
...
impl<T: Tile + Clone> Layout for TileWrapper<T> {
    type Schema = <T as Tile>::Schema;
    type Bundle = <T as Tile>::LayoutBundle;
    type Data = <T as Tile>::LayoutData;

    fn layout(
        &self,
        cell: &mut layout::CellBuilder<Self::Schema>,
            ...
            for (a, b) in path {
                ...
                if a.layer == b.layer {
                    ...
                    if track.width() > 0 && track.height() > 0 {
                        layout.draw(Shape::new(layer_stack.layer(a.layer).layer.clone(), track))?;
                    }
                    ...
                    if track.width() > 0 && track.height() > 0 {
                        layout.draw(Shape::new(layer_stack.layer(b.layer).layer.clone(), track))?;
                    }
...
pub(crate) fn get_abstract<T: Tile>(
    ...
    let TileData {
        ...
        outline,
        ..
    } = <T as Tile>::tile(&tile, &schematic_io, &mut cell)?;

...
pub trait Strapper: Send + Sync {
    ...
    fn strap(
        &self,
        routing_state: &mut RoutingState<AbstractLayer>,
...
struct GreedyStrapperState<'a, L> {
    routing_state: &'a mut RoutingState<L>,
    to_strap: Vec<(NetId, StrappingParams)>,
...
impl<'a, L: AtollLayer + Clone> GreedyStrapperState<'a, L> {
    fn new(
        routing_state: &'a mut RoutingState<L>,
        to_strap: Vec<(NetId, StrappingParams)>,
    ) -> Self {
        ...
        for (net, params) in self.to_strap.clone() {
            for layer in params.start..params.start + params.layers.len() {
                ...
                for i in tracks {
                    ...
                    if let Some(start) = start {
                        let stop = inner_end.checked_sub(std::cmp::max(1, via_space - 1));
                        if let Some(stop) = stop {
                            self.push_strap_if_valid(
                                Strap {
                                    net,

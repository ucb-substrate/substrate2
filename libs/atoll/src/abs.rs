...
use crate::grid::{
    AbstractLayer, AtollLayer, LayerSlice, LayerStack, PdkLayer, RoutingGrid, RoutingState,
};
use crate::{AssignedGridPoints, NetId, Orientation, PointState};
use grid::Grid;
use layir::{Shape, Text};
...
use substrate::layout::{CellBuilder, Draw, DrawReceiver, Layout};
...
impl Abstract {
    ...
    pub fn routing_state(&self) -> RoutingState<AbstractLayer> {
    ...
    pub fn from_routing_state(&mut self, state: RoutingState<AbstractLayer>) {
    ...
    pub(crate) fn slice(&self) -> LayerSlice<'_, AbstractLayer> {
        self.grid.slice()
    }
    ...
    pub fn generate<T: Schematic + Layout>(
        ctx: &Context,
        layout: &layout::Cell<T>,
        outline: Rect,
    ) -> Abstract {
        let stack = ctx
            .get_installation::<LayerStack<
                PdkLayer<<<T as Layout>::Schema as substrate::layout::schema::Schema>::Layer>,
            >>()
        ...
        let xmin = div_floor(outline.left(), slice.lcm_unit_width());
        let xmax = div_ceil(outline.right(), slice.lcm_unit_width());
        let ymin = div_floor(outline.bot(), slice.lcm_unit_height());
        let ymax = div_ceil(outline.top(), slice.lcm_unit_height());
        ...
        for (i, (_name, geom)) in cell.ports().enumerate() {
            ...
            for shape in geom.shapes() {
                if let Some(layer) = stack.layer_idx(shape.layer().clone()) {
...
fn top_layer<L: Eq + Clone>(cell: &RawCell<L>, stack: &LayerStack<PdkLayer<L>>) -> Option<usize> {
    let mut state = HashMap::new();
    top_layer_inner(cell, &mut state, stack)
}

fn top_layer_inner<L: Eq + Clone>(
    cell: &RawCell<L>,
    state: &mut HashMap<CellId, Option<usize>>,
    stack: &LayerStack<PdkLayer<L>>,
) -> Option<usize> {
    ...
    for elt in cell.elements() {
        match elt {
            ...
            Element::Shape(s) => {
                if let Some(layer) = stack.layer_idx(s.layer().clone()) {
...
#[derive(Debug, Clone)]
pub struct DebugAbstract<S: layout::schema::Schema> {
    /// The abstract to be written.
    pub abs: Abstract,
    /// The layer stack corresponding to the abstract.
    pub stack: LayerStack<PdkLayer<S::Layer>>,
}

impl<S: layout::schema::Schema<Layer: PartialEq>> PartialEq for DebugAbstract<S> {
    fn eq(&self, other: &Self) -> bool {
        self.abs == other.abs && self.stack == other.stack
    }
}
impl<S: layout::schema::Schema<Layer: Eq>> Eq for DebugAbstract<S> {}
impl<S: layout::schema::Schema<Layer: Hash>> Hash for DebugAbstract<S> {
...
impl<S: layout::schema::Schema<Layer: Eq + Hash>> Block for DebugAbstract<S> {
    type Io = ();
    fn name(&self) -> ArcStr {
...
impl<S: layout::schema::Schema<Layer: Clone + Eq + Hash>> Layout for DebugAbstract<S> {
    type Schema = S;
    type Bundle = ();
    type Data = ();
    #[inline]
...
impl<S: layout::schema::Schema<Layer: Clone>> Draw<S> for &DebugAbstract<S> {
    fn draw(self, recv: &mut DrawReceiver<S>) -> substrate::error::Result<()> {
        for (i, layer) in self.abs.layers.iter().enumerate() {
            let layer_id = self.stack.layer(i).layer.clone();
            match layer {
                LayerAbstract::Available => {}
                LayerAbstract::Blocked => {
                    recv.draw(Shape::new(layer_id.clone(), self.abs.physical_bounds()))?;
                }
                LayerAbstract::Detailed { states } => {
                    ...
                    for x in 0..tx {
                        for y in 0..ty {
                            ...
                            recv.draw(Shape::new(layer_id.clone(), rect))?;
                            let text = Text::with_transformation(
                                layer_id.clone(),
                                format!("({x},{y})"),

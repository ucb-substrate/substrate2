//! Utilities for byte-encoding and writing.
...
impl<'wr> GdsWriter<'wr> {
    /// Creates new [GdsWriter] with destination file `fname`.
    pub fn open(fname: impl AsRef<Path>) -> GdsResult<Self> {
        let file = BufWriter::new(File::create(fname)?);
        Ok(Self::new(file))
    }

    /// Creates a new [GdsWriter] to destination `dest`.
    ...
    /// Writes [GdsLibrary] `lib` to our destination.
    ...
    /// Encodes into bytes and write ontos `dest`.
...
/// An object that can be encoded in a GDS file.
///
...
trait Encode {
    ...
    /// Encodes a single [GdsRecord].
    fn encode_record(&mut self, record: GdsRecord) -> GdsResult<()>;
    /// Encodes an array of [GdsRecord]s.
    fn encode_records(&mut self, records: &[GdsRecord]) -> GdsResult<()>;

    // Default Methods
    /// Encodes a [GdsLibrary].
    ...
    /// Encodes a [GdsElement], dispatching across its variants.
...
impl GdsDateTimes {
    /// Encodes a date-time in GDSII's vector of i16's format.

...
struct Merger<'a, P> {
    ...
    names: Names<CellId>,
    dst: &'a mut LibraryBuilder<P>,
    src: &'a LibraryBuilder<P>,
}

impl<'a, P> Merger<'a, P> {
    #[inline]
    fn new(dst: &'a mut LibraryBuilder<P>, src: &'a LibraryBuilder<P>) -> Self {
        Self {
        ...
        if let CellKind::Cell(inner) = n_cell.contents_mut() {
            for (_, inst) in inner.instances_mut() {
                let n_child_id = self.merge_cell(
                    ...
                    self.src.cell(inst.child().unwrap_cell()),
                );
                inst.child = n_child_id.into();
            }
        }
